...
data COLUMN;
set SASHELP.VCOLUMN(where=(memname="OUT_MODELING_READY" and label="CAT_FRANCE"));
call symput("categ",name);
run;

data casuser.&langue._CATEG;
merge CASUSER.OUT_MODELING_READY(where=(&categ.=1) in=in1) Public.&table_name.(keep=socmedia_id &varname.);
by socmedia_id;
if in1;
run;
...
data Public.CONCEPTS_CATEGRANCE;
set Public.EN_CONCEPTS_CATEGFRANCE Public.FR_CONCEPTS_CATEGFRANCE;
run;

data Public.FULL_CONCEPTS_CATEGFRANCE(promote=yes);
merge PG_NATO.ENT_SOCMEDIAS Public.EN_CONCEPTS_CATEGFRANCE(in=in1);
by socmedia_id;
if in1;
run;

data Public.FACTS_CATEGRANCE(promote=yes);
set Public.FACTS_FRCATEG Public.FACTS_ENCATEG;
run;
